...
def _register_thai_fonts():
    ...
    try:
        ...
        if registered_fonts:
            ...
            print("No bundled report fonts found, using default")
...
def _create_header_footer(canvas, doc):
    ...
    canvas.drawString(2*cm, 1.5*cm, f"Page {page_num}")
...
def _create_kpi_cards(df, columns_to_analyze):
    ...
    # Create KPI table
    headers = ["Column", "Count", "Mean", "Std Dev", "Minimum", "Maximum"]
...
def export_report(fig, df: pd.DataFrame, meta: dict, save_path: str, options: dict = None):
    ...
    if not options["title"].strip():
        options["title"] = "SciPlotter Data Analysis Report"
    
    try:
        # Register bundled fonts when available.
        fonts = _register_thai_fonts()
        report_font = list(fonts.keys())[0] if fonts else "Helvetica"
        report_bold_font = "THSarabunNew Bold" if "THSarabunNew Bold" in fonts else report_font
        ...
        story.append(Paragraph(f"Created: {timestamp}", subtitle_style))
        ...
        if options["include_meta"]:
            ...
            meta_data.append(["File name", filename])
            
            if 'columns_used' in meta and meta['columns_used']:
                columns_used = meta.get('columns_used', [])
                meta_data.append(["Plot columns", ", ".join(columns_used)])
            
            meta_data.append(["Rows", f"{len(df):,}"])
            meta_data.append(["Columns", f"{len(df.columns):,}"])
            
            # Add dataset name if available
            if 'dataset_name' in meta:
                meta_data.append(["Dataset", meta['dataset_name']])
            ...
            meta_table.setStyle([
                ...
                ('FONTNAME', (0, 0), (0, -1), report_bold_font),
            ])
            ...
            try:
                ...
                if kpi_data:
                    ...
                    kpi_table.setStyle([
                        ...
                        ('FONTNAME', (0, 1), (0, -1), report_bold_font),  # Column names bold
                ...
                table_data = [["Column", "Count", "Mean", "Std Dev", "Minimum", "25%", "50%", "75%", "Maximum"]]
                ...
                story.append(Paragraph(f"Statistics section failed: {str(e)}", normal_style))
                story.append(Spacer(1, 20))
        ...
        if options["include_fig"]:
            ...
            try:
                ...
                story.append(Paragraph("Figure 1: Data analysis graph", caption_style))
                
            except Exception as e:
                story.append(Paragraph(f"Graph section failed: {str(e)}", normal_style))
                story.append(Spacer(1, 20))
