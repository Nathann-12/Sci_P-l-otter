...
- **UX ทั้งแอปยึดโมเดล 3 ขั้น** (ดู [docs/UX_FLOW.md](docs/UX_FLOW.md) — บังคับสำหรับฟีเจอร์ใหม่): panel ซ้าย = การ์ด ①ข้อมูล ②เลือกคอลัมน์→พล็อต (CompactPlotPanel อยู่ที่นี่ ไม่ใช่ใน Inspector) ③เครื่องมือกราฟ; **ห้ามซ่อนความสามารถหลักไว้ใน dock/panel ที่ปิดอยู่**
...
- **worksheet แบบ Excel** ([widgets/workbook.py](widgets/workbook.py) `WorkbookWidget`) — คอลัมน์ A(X)/B(Y), แถว Long Name/Units/Comments/F(x)=; `self.workbook` มิเรอร์ `self._df` (ดู `_refresh_workbook`) **และเป็นจุดเริ่ม workflow ได้เอง**: แถบเครื่องมือ/คลิกขวาบนชีต emit `use_data_requested`/`plot_requested` → `adopt_workbook_data` (data mixin) / `plot_from_workbook` (plot mixin)
