...
- ✅ Surface plot (3D)
- ✅ Matrix workflow (เมนู Matrix: XYZ→Matrix gridding nearest/linear/cubic + exact pivot, Matrix→XYZ, Import Image as Matrix, transpose/flip/rotate/crop/resize, NaN-aware Gaussian/median smooth, background subtract รวม plane fit, normalize/clip, **2D FFT magnitude**, **matrix statistics** (min/max/mean + พิกัด extrema), **line profile** ตามเส้น (ลากพิกัด → กราฟ Z), **matrix arithmetic** A⊕B (subtract/add/multiply/divide, difference image), Heatmap/Contour/3D Surface ด้วยพิกัดจริง — AI `grid_xyz`/`matrix_transform`/`matrix_statistics`/`line_profile`/`matrix_arithmetic`/`plot_matrix`)
