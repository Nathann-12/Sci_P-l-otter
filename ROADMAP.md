...
- ✅ Batch export
- ✅ Layout page — free-form composition canvas (Report → Layout Page / Ctrl+Shift+L): ลาก/ปรับขนาด/snap grid วางกราฟ+ตาราง+ข้อความอิสระบนหน้า (A4/Letter/Slide 16:9), Auto Arrange, export **PDF/PNG** WYSIWYG; AI `arrange_layout` จัดกริดอัตโนมัติเป็นโปสเตอร์
- ✅ One-click publication report (Report menu / Ctrl+Shift+R — รวม Graphs + result Books + auto data summary เป็นเอกสารเดียว → **self-contained HTML** (ฝังภาพ base64, พิมพ์เป็น PDF ได้เนียน), **PDF** (reportlab), **Word DOCX** (python-docx), **PowerPoint PPTX** (python-pptx — สไลด์ต่อ figure/table), **Markdown**; Report Builder เลือก title/author/template/what-to-include, Quick Report คลิกเดียว, AI tool `generate_report`)
...
## Latest verification
- 2026-07-19: Multi-layer Y axes (Origin "layers") — `add_y_axis_layer(ax, x, y, index=…)` in `core/plot_extras.py` stacks independent right-hand Y scales (2nd+ spine offset outward, axis/ticks/label colour-matched to the curve); `count_extra_y_axes(ax)` counts existing twin-Y layers so each new one offsets correctly. Mixin `plot_y_axis_layer()` (Plot → Add Y-Axis Layer…) picks a column via the form seam, adds the layer to the current graph and registers it. Verified 3-scale render + real-MainWindow flow (2 stacked layers, no new graph). `tests/test_plot_extras.py` +3 tests (core offset/colour, mixin stack, needs-primary-curve); 15 passed.
- 2026-07-19: QA bug-hunt pass — exhaustive real-use sweep (every menu action ×277, toolbar ×122, gallery 60 charts × 10 edge datasets = 600 combos, cross-feature lifecycle ×6) found one real bug: the Report Builder dialog crashed on open + silently dropped all tables (PySide6 strict-enum `setCheckState(2)` / `checkState()==2`). Fixed to `Qt.Checked`; whole Generate Report flow verified end-to-end. Regression test added.
- 2026-07-19: Color-mapped line plot — an Origin "color mapped" line whose colour varies along its length via a `LineCollection`: colours by a 3rd numeric column, else by Y (2 cols) or row index (1 col), with a colourbar. Two gallery entries (`color_mapped_line`, `color_mapped_line_markers`, category Multi-Column) in `plots/colormap_plots.py`, registered in `plots/registry.py` so it rides the Charts gallery and the AI `plot_chart`/`list_charts` tools automatically (registry now 60 charts, no new AI tool / no corpus re-seal). Degenerate/non-numeric input draws a placeholder. `tests/test_colormap_plots.py` (6) + gallery sweep: 77 passed.
- 2026-07-19: Decoration parity with Origin — added per-line pattern (hatch) fill, gradient fill (imshow clipped to the fill poly, fades to baseline), drop lines (LineCollection to y=0, auto-thinned), and max/min mark+label. All gid-tagged (`_ps_fill/_ps_drop/_ps_extrema`), re-apply replaces, toggle-off removes, round-trips via read_line_style; dialog Lines-tab controls + live preview + Cancel-revert verified. `_remove_gid_artists` sweeps `ax.images`. `tests/test_plot_decorations.py` +5 tests; decorations+palettes+style sweep 55 passed.
- 2026-07-19: Annotation parity with presentation apps — resize handles (line/arrow endpoints, rect/ellipse corners, callout tip+text) with anchored-opposite-corner math and undo; Ctrl+D duplicate + Ctrl+C/V copy-paste (3%-offset clones); arrow-key nudge (Shift = coarse, key-repeat coalesced into one undo entry); PgUp/PgDn bring-to-front/send-to-back; Shift-constrained drawing (lines/arrows snap 45° in display space, rect/ellipse become square/circle); right-click context menu on items (Edit Text/Duplicate/Front/Back/Delete) with the graph context menu yielding via `consumes_right_click`. `tests/test_annotations_interactive.py` 16 passed; GUI sweep 128 passed.
- 2026-07-19: Annotation refactoring — high-performance coordinate-based selection, drag-to-move, escaping, canvas key events, inline editing, and live style/selection sync with dock. Switch FancyArrow to mutable FancyArrowPatch (recreates zeroed out) and implement Callout using ax.annotate. Exclusive QActionGroup groups drawing actions for armed status feedback. Sync toolbar buttons to share menu QAction. Headless coverage added for interactive actions; full 129 test files passed (1281 + 3 new passed, 3 skipped).
- 2026-07-18: Report export completed — Word (DOCX) + PowerPoint (PPTX) and document-format batch reports. `core/report.py` gains `render_docx` (headings/figures/tables/KPIs via python-docx) and `render_pptx` (title slide + one slide per figure/table/text via python-pptx) from the same ReportDocument model; `report_export` + the Report menu + the AI `generate_report` `format` enum now include docx/pptx (still one tool, registry stays 58 — no corpus re-seal). `analysis/batch.py` gains `batch_report_document` (KPIs + per-file summary table + failures) and `export_batch_report` now also renders PDF/DOCX/PPTX, wired into the Batch Analysis dialog's format picker. Added python-docx/python-pptx to requirements. New DOCX/PPTX renderer tests + batch document-format tests; report+batch+training sweep `133 passed`.
- 2026-07-18: Layout page — a free-form composition canvas (the OriginPro "layout window", but easier). Pure `core/layout.py` holds a page model (A4/Letter/Slide-16:9 presets, figure/table/text items in points) + a reportlab **PDF** renderer and a matplotlib **PNG** renderer + an `auto_grid` that tidily fills the page. `UI/layout_page.py` is an interactive `QGraphicsView` editor: drag-move, corner-resize, snap-to-10px-grid, add figures/tables (from open Graphs/Books) and text, Auto Arrange, delete, page-size switch, and WYSIWYG export to PDF/PNG (serialises the scene back to the pure model, so the file matches the canvas). Opens pre-filled and auto-arranged so it's never blank (Report → Layout Page / `Ctrl+Shift+L`). New AI tool `arrange_layout` auto-composes a poster and exports — registry 57 → **58**, router corpus regenerated + re-sealed (release-v3 `e6a70cc1…`, acceptance-v4 `c37d59ef…`). New `tests/test_layout_core.py` (6) + `tests/test_layout_editor.py` (7, incl. QGraphicsView editor + AI); layout+report+training sweep `69 passed`; both `--check` gates green; 2×2 poster demo rendered.
