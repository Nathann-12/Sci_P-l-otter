...
- ✅ Surface plot (3D)
- ✅ Matrix workflow (เมนู Matrix: XYZ→Matrix gridding nearest/linear/cubic + exact pivot, Matrix→XYZ, Import Image as Matrix, transpose/flip/rotate/crop, NaN-aware Gaussian/median smooth, background subtract รวม plane fit, normalize/clip, Heatmap/Contour/3D Surface ด้วยพิกัดจริง — AI `grid_xyz`/`matrix_transform`/`plot_matrix`)
