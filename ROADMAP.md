...
- ✅ Surface plot (3D)
- ✅ Matrix / Image / Surface workflow (เมนู Matrix เต็ม: XYZ↔Matrix gridding + exact pivot, Import Image, geometry/smooth/background/normalize/clip/resize, **2D FFT**, **threshold/edge/contrast/morphology**, **ROI ตามพิกัดจริง**, **matrix statistics + surface metrics** (Ra/Rq/volume/slope), **line profile**, **gradient/slope map**, **matrix arithmetic** A⊕B, **stack projection** (max/mean/... หลายเฟรม), Heatmap/Contour/3D Surface ด้วยพิกัดจริง — AI 8 tools: grid_xyz/matrix_transform/matrix_statistics/line_profile/matrix_arithmetic/plot_matrix/surface_metrics/matrix_stack)
...
- ✅ Batch export
- ✅ One-click publication report (Report menu / Ctrl+Shift+R — รวม Graphs + result Books + auto data summary เป็นเอกสารเดียว → **self-contained HTML** (ฝังภาพ base64, พิมพ์เป็น PDF ได้เนียน), **PDF** ตรงผ่าน reportlab, **Markdown**; Report Builder เลือก title/author/template/what-to-include, Quick Report คลิกเดียว, AI tool `generate_report`)
...
- ✅ สรุปรายงานอัตโนมัติ (AI tool `generate_report` — รวมกราฟ+ตาราง+auto summary เป็น HTML/PDF/Markdown)
...
## Latest verification
- 2026-07-18: Report / Publishing — one-click publication reports that beat Origin's Word/PPT flow on ease. Pure `core/report.py` holds a document model (title/kpi/text/figure/table sections) with three renderers: **self-contained HTML** (images embedded as base64, escaped, print-to-PDF-clean, theme-light), **Markdown** (writes figure PNGs + GFM tables), and **PDF** via reportlab platypus. `main_window_report_mixin.py` collects every open Graph (rendered to PNG) + result Book (as a table) + an auto data-driven narrative into a `ReportDocument`; a **Report menu** (Generate Report… `Ctrl+Shift+R` / Quick Report / Export HTML·PDF·Markdown), a small **Report Builder** dialog (title/author/template + include toggles + per-table checklist, Preview or Export), and preview via the system browser. New AI tool `generate_report` assembles + saves from the current session — registry 56 → **57**, router corpus regenerated + re-sealed (release-v3 `4a441951…`, acceptance-v4 `86eb378b…`). New `tests/test_report_core.py` (7) + `tests/test_report_mixin.py` (7, incl. QAction menu + AI); report+AI+training sweep `165 passed`; both `--check` gates green; HTML/PDF demos rendered.
- 2026-07-18: Matrix module completed — image processing, ROI, surface analysis and matrix stacks. `analysis/matrix_ops.py` adds `threshold` (binary/mask/to_zero), `edge_detect` (sobel/prewitt/laplace gradient magnitude), `contrast` (brightness/contrast about the mean), `morphology` (grayscale erode/dilate/open/close), `extract_roi` (crop by DATA coordinates), `surface_metrics` (Ra/Rq roughness, peak-to-valley, volume-above-min, slope stats — numpy-2 `trapezoid`), `gradient_magnitude` (physical-spacing slope map), and `stack_project` (max/mean/min/sum/std across equally-shaped frames — per-pixel NaN-aware, e.g. maximum-intensity projection). New Matrix menu sections **Image** (Threshold / Edge / Brightness-Contrast / Morphology / Extract ROI) and expanded **Analyze** (Surface Metrics / Gradient / Stack Projection). Six new ops ride the existing `matrix_transform` AI tool; two genuinely new tools (`surface_metrics`, `matrix_stack`, registered append-only) take the registry 54 → **56**, with the router corpus regenerated + re-sealed (release-v3 `c398a51f…`, acceptance-v4 `d5aa1a8a…`). New pure tests (8) + mixin/AI tests; matrix+AI+training sweep `138 passed`; both `--check` gates green; 5-panel image/surface visual demo verified.
- 2026-07-18: Matrix analysis depth added (line profile / matrix arithmetic / matrix statistics / 2D FFT + resize). `analysis/matrix_ops.py` gains `statistics` (min/max/mean/median/std/sum + X/Y coordinates of the extrema), `fft2_magnitude` (centred log spectrum, NaN-filled), `combine` (subtract/add/multiply/divide with a shape guard and 0/0→NaN), `line_profile` (bilinear `map_coordinates` sampling along p0→p1 in data coordinates, NaN-aware so holes read NaN not interpolated across), and `resize` (spline resample carrying large holes). Matrix menu gains an **Analyze** submenu (Statistics / Line Profile / Matrix Arithmetic / 2D FFT) plus Resize; line profile opens a Book **and** plots the curve, arithmetic picks a second matrix Book. Three new AI tools (`matrix_statistics`, `line_profile`, `matrix_arithmetic`) + fft2/resize added to `matrix_transform`; registry 51 → **54 tools** and the router corpus was regenerated + re-sealed (release-v3 `90eb17f0…`, acceptance-v4 `9ab26c76…`). New pure tests (8) + mixin/AI tests; consolidated matrix+AI+training sweep `180 passed`; both `--check` gates green.
- 2026-07-18: Format-Graph redesign after screenshot feedback ("คลิกเส้นมันเล็กไป"). **Double-click anywhere on the graph now opens Plot Details** — no aiming at thin lines; landing within a generous 12px pick radius of a curve preselects it, and Ctrl/Shift+double-click opens the Graph Data inspector instead. Root cause found for the missing right-click entries: `plot_tabs` resolved MainWindow via `parent().parent()`, which stopped working when graphs moved into MDI subwindows — `ContextMenuManager` now lazily walks the widget chain (`_resolve_main`), so the menu's new **bold top entry "Format Graph…  (double-click)"** + "Graph Data (Inspector)" always appear; `_build_menu` split from `_show_menu` for headless testability with a real-window regression test. Suites: 58 passed.
- 2026-07-18: Discoverability pass from real-use feedback ("สถิติไม่มีปุ่ม / หน้าตกแต่งไม่เด้ง"). (1) **Scientific suite + Matrix now have toolbar buttons** on the bottom Process & Analyze dock — new labelled groups `Scientific` (Statistics / Global Fit / Peak Analyzer / Recipes / Recalculate / Batch) and `Matrix` (XYZ→Matrix / Heatmap / Surface), all registered in `toolbar_actions`, data-dependent keys dim via `_ACTIONS_NEED_DATA`, qtawesome icons added. (2) **Double-click a curve now opens Plot Details focused on that curve** (no Ctrl needed — Origin muscle memory); blank-canvas double-click keeps Graph Data; Ctrl+double-click still always opens Plot Details; the graph right-click menu gains top-level "Format Graph… (Plot Details)" and "Graph Data (Inspector)". (3) Result Books re-assert MDI activation on the next event-loop turn so a closing modal dialog can't hide a fresh statistics result behind a maximized Graph. Updated workbook-flow/side-toolbar tests; affected suites `88 passed` + toolbar suites `33 passed`.
- 2026-07-18: Matrix / Image / Surface first production pass — the "สูงมาก" gap row now has a real engine. Pure cores: `analysis/gridding.py` (XYZ→matrix with an exact pivot fast-path for complete rectangular grids, scipy `griddata` nearest/linear/cubic for scattered data, hull cells stay NaN and are **counted, never invented**; matrix Books encode coordinates in the column labels + a leading `y` column so they round-trip through sessions with no side-channel metadata) and `analysis/matrix_ops.py` (transpose/flip/rotate90/crop with coordinate bookkeeping, **NaN-aware** Gaussian smoothing via normalized convolution, median filter, background subtraction incl. least-squares plane fit, min-max/z-score normalize, clip, and image→luminance-matrix import). New top-level **Matrix menu** (`main_window_matrix_mixin.py`: Convert / Transform / Filter && Background / Visualize) whose handlers collect params via `ask_form` and call param-taking cores (`matrix_grid_core`/`matrix_transform_core`/`matrix_plot_core`) — the same cores the **3 new AI tools** (`grid_xyz`, `matrix_transform`, `plot_matrix`) call with no dialogs; matrix plots draw with real X/Y coordinates (pcolormesh/contourf/3D surface via the projection seam) plus colorbars. Registry 48 → **51 tools**; the router corpus was regenerated + re-sealed per the documented procedure (release-v3 `893424f0…`, acceptance-v4 `57cb13af…`; language-balance audits now allow |en−th| ≤ 1 since 63 records cannot split evenly). New `tests/test_matrix_core.py` (11) + `tests/test_matrix_mixin.py` (7, incl. QAction menu flow and end-to-end AI); training/catalog suites green; both dataset `--check` gates pass; 4-panel visual pipeline demo verified (scattered → gridded → filtered → surface).
